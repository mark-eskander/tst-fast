...
with zipfile.ZipFile('classifier.zip', 'r') as zip_ref:
...
with zipfile.ZipFile('vectorizer.zip', 'r') as zip_ref:
     with zip_ref.open('vectorizer.pkl') as file:
    # Load the object from the pickle file
        vectorizer = pickle.load(file)
...
def cleaned_tokenized(x):
    ...
    result=int(model.predict(result)[0])
    
    # Use the model to predict the sentiment
    # result = "positive Review 😀" if int(model.predict(result)[0]) == 1 else "Bad Review 😔" 

    return result

# print(type(cleaned_tokenized("this-is-good")))